...
<div x-data="{
    ...
    <button type="button" @click="isOpen = !isOpen" class="flex items-center justify-between w-full px-4 h-11 text-left border-2 border-zinc-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
    ...
    <div x-show="isOpen" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700 dark:text-zinc-300 border rounded shadow-lg max-h-60 overflow-y-auto" @click.away="isOpen = false">
        ...
        <!-- Checkbox for select/deselect all options -->
        ...
        <template x-for="(option, index) in filteredOptions" :key="index">
            <label class="flex items-center px-4 py-2">
                <input type="checkbox" x-model="selectedOptions" @change="updateSelectAllState" :value="option" class="form-checkbox h-5 w-5 text-indigo-600">
...
<script>
    ...
    function toggleSelectAll() {
        if (this.selectAll) {
            ...
            this.selectedOptions = this.selectedOptions.filter(option => !this.filteredOptions.includes(option));
        }
    }

    function updateSelectAllState() {
        this.selectAll = this.filteredOptions.length > 0 && this.filteredOptions.every(option => this.selectedOptions.includes(option));
    }
</script>
